...
module.exports = {
  ...
  MERKLEPROVE: function MERKLEPROVE (proof, proofLen, leaf, root, runState) {
    ...
    if (computedHash.equals(root.toBuffer())) {
      // runstate, start, val, valOffset, valLength, skipSubMem(opt)
      memStore(runState, proof.add(proofLen), new BN(1).toBuffer(), new BN(0), new BN(1))
    } else {
      memStore(runState, proof.add(proofLen), new BN(0).toBuffer(), new BN(0), new BN(1))
    }
    return proof.add(proofLen)
  }
}
