...
module.exports = {
  ...
  GETATTEST: function (filters, key, runState, cb) {
    ...
    stateManager.getContractStorage(runState.address, key, function (err, value) {
      console.log('Storage value: ', new BN(value).toString('hex'))
      if (err) return cb(err)
      value = value.length ? new BN(value) : new BN(0)
      let word = value.toArrayLike(Buffer, 'be', 32)
      // Use the first available slot
      let offset = runState.memoryWordCount
      console.log('Offset:', offset.toString())
      // TODO: Need to write more than one word
      // TODO: subtract gas required for memory
      memStore(runState, offset, word, new BN(0), new BN(32))
      // Add start = of attestation to stack
      var topIndex = runState.stack.length - 1
      // TODO: is this safe? possible stack underflow?
      runState.stack[topIndex + 1] = offset
    })
  },
