...
module.exports = {
  ...
  /*
     ...
     * root - root of trie
     */
  MERKLEPROVE: function MERKLEPROVE (proof, proofLen, leaf, root, runState) {
    if (!proof) {
      throw new Error('Missing opcode param: proof')
    }
    if (!proofLen) {
      throw new Error('Missing opcode param: proofLen')
    }
    if (!leaf) {
    ...
    if ((proofLen - 32) % 33 !== 0) {
      // trap(ERROR.REVERT)
      throw new Error('MERKLEPROVE: invalid proof length: ' + proofLen)
    }

    // TODO: Implement gas cost function

    var i, computedHash, proofElement, word, isRight
    computedHash = leaf

    for (i = 33; i <= proofLen; i += 33) {
      proofElement = memLoad(runState, proof.add(new BN(i)).mod(utils.TWO_POW256), new BN(32))
      // Load from proof + (i - 1)
      word = memLoad(runState, proof.add(new BN(i - 1)).mod(utils.TWO_POW256), new BN(32))
      // Get first byte
    ...
    if (computedHash.equals(root.toBuffer())) {
      return new BN(1)
    }
    return new BN(0)
  }
