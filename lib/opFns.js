...
module.exports = {
  ...
  MERKLEPROVE: function MERKLEPROVE (proof, leaf, root, runState) {
    // Load proofLen from first 32bytes of proof (bytes)
    let proofLen = memLoad(runState, proof, new BN(32))
    // Ignore first 32 bytes for length
    // if proofLen % 33 != 0
    if (!new BN(proofLen).mod(new BN(33)).eq(new BN(0))) {
      // trap(ERROR.REVERT)
      throw new Error('MERKLEPROVE: invalid proof length: 0x' + proofLen.toString('hex'))
    }
    ...
    if (computedHash.eq(root.toBuffer())) {
      return new BN(1)
    }
    return new BN(0)
  }
