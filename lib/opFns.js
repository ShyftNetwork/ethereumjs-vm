...
module.exports = {
  ...
  MERKLEPROVE: function MERKLEPROVE(proof, leaf, root, runState) {
    ...
    // if proofLen % 33 != 0
    if (!proofLen.mod(new BN(32)).eq(new BN(0))) {
      trap(ERROR.REVERT)
    }

    var computedHash, proofElement, word, appendRight;
    computedHash = leaf;
    // Read proof[32:63] for indicators
    var indicatorBuffer = new BN(memLoad(runState, proof.add(new BN(32)), new BN(32)), 16).toString()
    var indicators = parseInt(indicatorBuffer)
    var i = new BN(64);

    while (i.lte(proofLen)) {
      proofElement = memLoad(runState, proof.add(i).mod(utils.TWO_POW256), new BN(32));
      // Check first bit
      appendRight = indicators & 1;
      indicators = indicators >>> 1
      if (appendRight === 1) {
        computedHash = utils.sha3(new BN(computedHash.toString('hex') + proofElement.toString('hex'), 16));
      } else if (appendRight === 0) {
