...
module.exports = {
  ...
  GETATTEST: function (a, b, runState, cb) {
    cb(null, a.add(b).mod(utils.TWO_POW256))
  },
  GETREVOKE: function (a, b) {
    return a.add(b).mod(utils.TWO_POW256)
  },
  CHECKATTESTVALID: function (a, b) {
    return a.add(b).mod(utils.TWO_POW256)
  },
  /*
     * proof - starting location of proof in memory
     * proofLen - size of proof
     * lead - hashed leaf value
     * root - root of trie
     */
  MERKLEPROVE: function MERKLEPROVE (proof, leaf, root, runState) {
    if (!proof) {
      throw new Error('Missing opcode param: proof')
    }
    if (!leaf) {
      throw new Error('Missing opcode param: leaf')
    }
    if (!root) {
      throw new Error('Missing opcode param: root')
    }

    var proofLen = new BN(memLoad(runState, proof, new BN(32)), 16)
    // Ignore first 32 bytes for length
    // if proofLen % 33 != 0
    if (!proofLen.mod(new BN(33)).eq(new BN(0))) {
      // trap(ERROR.REVERT)
      throw new Error('MERKLEPROVE: invalid proof length: ' + proofLen.toString() + ' (proofLen - 32) % 33: ' + new BN(proofLen).sub(new BN(32)).mod(new BN(33)))
    }

    // TODO: Implement gas cost function

    var computedHash, proofElement, word, isRight
    computedHash = leaf

    let i = new BN(33)

    while (i.lte(proofLen)) {
      proofElement = memLoad(runState, proof.add(i).mod(utils.TWO_POW256), new BN(32))
      // Load from proof + (i - 1)
      word = memLoad(runState, proof.add(i.sub(new BN(1))).mod(utils.TWO_POW256), new BN(32))
      // Get first byte
      isRight = word.toString('hex').substr(0, 2)
      if (isRight === '01') {
        computedHash = utils.sha3(new BN(computedHash.toString('hex') + proofElement.toString('hex'), 16))
      } else {
        computedHash = utils.sha3(new BN(proofElement.toString('hex') + computedHash.toString('hex'), 16))
      }
      i = i.add(new BN(33))
    }

    if (new BN(computedHash).eq(root)) {
      return new BN(1)
    }
    return new BN(0)
  }
