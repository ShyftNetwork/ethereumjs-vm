...
module.exports = {
  ...
  MERKLEPROVE: function (proof, proofLen, leaf, root, runState){
    if(proofLen % 32 !== 0){
      trap(ERROR.REVERT)
    }

    let i, computedHash, proofElement
    //TODO: Does this need to be hashed?
    computedHash = leaf

    for(i = 0; i < proofLen; i += 32){
      console.log(`i: ${i}`)
      console.log(`proof: ${proof}`)
      console.log(`proofLen: ${proofLen}`)
      console.log(`Root: ${root.toString('hex')}`)
      console.log(`Leaf hash: ${computedHash.toString('hex')}`)
      proofElement = memLoad(runState, proof.add(new BN(i)).mod(utils.TWO_POW256), new BN(32))
      console.log(`Current element: ${proofElement.toString('hex')}`)
      if (new BN(computedHash).lt(new BN(proofElement))) {
        console.log(`${computedHash.toString('hex')} < ${proofElement.toString('hex')}`)
        computedHash = utils.sha3(new BN(computedHash.toString('hex') + proofElement.toString('hex'), 16));
        console.log(`New hash: ${computedHash.toString('hex')}`)
      } else {
        console.log(`${computedHash.toString('hex')} >= ${proofElement.toString('hex')}`)
    ...
    console.log(computedHash.equals(root.toBuffer()))
    return computedHash.equals(root.toBuffer())
  }
}
