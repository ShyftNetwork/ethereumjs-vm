...
// @note: @chris: the following is an extension on prior work of myself
// to figure out what use a dictionary/phrase temporal indexing could
// possibly have except in cases where the bandwidth is (incredibly)
// limited. It seemed to be of little practical usage so I'm going
// to connect this with Shyft stuff. Anyone that reads this message
// has assumed the liability of one ball of golden yarn (tumbleweed size
// but not mass please, whether it's less mass or more to follow 'Good
// Genie Rules') to be delivered to me upon the end of time (or
// delivered upon me to the end of time, again 'Good Genie Rules'), or
// sooner if you want to clear your conscience.

// Indeed, the amount of reduction can be the equivalent of the number
// of (special purpose) registered names (as these indexes are expected
// to be standardized) times the total number of keys in the KYC Matrix
// plus a predefined use purpose where people invent cool new names for
// things. Such a dense formulation dictionary would morph over
// time as additional standards were added, but at a very slow pace...
// 2^32 is a lot of unsigned integers, and we can put hashes in our
// dictionary too of course to other dictionaries over time, and so on.
//
// Cool things such as bloom filters can be done on such dictionaries to
// prove compatibility with the existing dictionary (that is, the order
// of the word is unchanged, and the word itself is unchanged) or
// existence (which could be a false positive, such that the iteration
// time would be the worst cast scenario in n of m lookups into the
// bloom compatibility filter). Multiple bloom fields could correlate
// this data as it is the expected use pattern (existence, position,
// word are unrelated variables) and be an extremely efficient lookup
// system as only in the case of complete correlation would the bloom
// patterns match.
// Any shifts to the underlaying dictionary would result in a complete
// disambiguation from existing EVM blockchains, and yet a mapping
// system could still be written as an attested smart contract push
// with a current-state of the Shyft blockchain.
