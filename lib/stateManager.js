...
proto._putAccount = function (address, account, cb) {
  ...
  // @note: @todo: @here: for Hypothetical, set up debt for raw account, perhaps halve the deposit required
  // for accounts that have an ID associated with them (would need to have a *pederson commitment* signature
  // from the ID'd main account.
...
proto.getAccountBalance = function (address, cb) {
  var self = this
  // @note: @todo: @here: for Hypothetical, get balances as a list with a collateralization spin function.
...
proto.putVerificationContract = function (address, value, cb) {
  var self = this
  self.getAccount(address, function (err, account) {
    if (err) {
      return cb(err)
    ...
    // TODO: setCode use trie.setRaw which creates a storage leak
    // @note: @here: @deepinspect: is this actually true for a simple address to a contract?
...
proto.getVerificationContract = function (address, cb) {
  var self = this
  self.getAccount(address, function (err, account) {
    if (err) {
      return cb(err)
...
// creates a identity trie from the primary storage trie
proto._lookupIdentityTrie = function (address, cb) {
  var self = this
  // from state trie
  self.getAccount(address, function (err, account) {
    if (err) {
    ...
    var identityTrie = self.trie.copy()
    identityTrie.root = account.stateRoot
    self.touched.push(address)
    identityTrie._checkpoints = []
    cb(null, identityTrie)
  })
}

// gets the identity trie from the storage cache or does lookup
proto._getIdentityTrie = function (address, cb) {
  var self = this
  var identityTrie = self._identityTries[address.toString('hex')]
  // from storage cache
  if (identityTrie) {
    return cb(null, identityTrie)
  }
  // lookup from state
  self._lookupIdentityTrie(address, cb)
}
